...
public class EventService {
    ...
    public Event saveEvent(Event event) {
     ...
     /*   DateFormat outputFormat = new SimpleDateFormat("yyyy-MM-dd");
        Date date = outputFormat.parse(event.getDate());
        String outputText = outputFormat.format(date);
        event.setDate(outputText);

        String originalString = "2022-06-03T19:32:26.757Z";
        Date date2 = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").parse(event.getStart_time());
        String newString = new SimpleDateFormat("h:mm a").format(date2);

        event.setStart_time(newString);*/
    ...
    public ResponseEntity<Event> updateEvent(Event event ) {
        ...
        if (event.getCountry()!=null) existingEvent.setCountry(event.getCountry());
